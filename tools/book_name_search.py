...
def build_book_search_query(search_config: Dict[str, Any]) -> Dict[str, Any]:
    """根据搜索配置构建Elasticsearch查询（专门用于图书搜索），使用should组合所有搜索类型"""
    query_text = search_config.get("query", "")
    
    # 构建should查询，组合所有搜索类型
    should_queries = [
        # 1. 前缀搜索
        {"prefix": {"slotReName": query_text}},
        # 2. 全名匹配
        {"term": {"slotReName": query_text}},
        # 3. 包含搜索（模糊匹配）
        {
            "match": {
                "slotReName": {
...
@tool
def book_name_search_tool(search_config: str, index: str = "slots_index") -> str:
    """
    在 Elasticsearch 中执行图书名称搜索，组合多种搜索方式。
    
    该工具专门用于搜索图书名称，同时使用前缀搜索、全名匹配和包含搜索。
    ...
    try:
        ...
        # 构建搜索查询（专门搜索图书名称）
        search_query = build_book_search_query(config)
        
        # 执行搜索，限制结果数量为10条
        results = es_client.search(index=index, query=search_query, size=10)
        
        if not results:
            return f"未找到与 '{query_text}' 相关的图书信息"
